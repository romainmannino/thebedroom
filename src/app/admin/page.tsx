...
export default function AdminPage() {
  ...
  function prepareStay() {
    if (!guestName.trim() || !arrivalDate || !departureDate) {
      window.alert("Renseigne au minimum le prénom, la date d’arrivée et la date de départ.");
      return;
    }
  ...
  async function shareOrCopyLink() {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "Votre séjour à The Bedroom", text: message });
    ...
    <main className="min-h-screen bg-[#e7dfd4]">
      <div className="mx-auto min-h-screen w-full max-w-[1280px] bg-[#faf8f4]">
        <header className="flex items-center justify-between border-b border-black/5 px-5 py-4 sm:px-8">
          ...
          <Link href="/" className="grid h-10 w-10 place-items-center rounded-full bg-black text-white" aria-label="Voir le livret"><Home size={18} /></Link>
        </header>
        ...
        <div className="grid lg:grid-cols-[215px_1fr]">
          <aside className="hidden border-r border-black/5 p-4 lg:block">
            <nav className="space-y-1.5">
              ...
              <AdminLink href="/admin/minibar" icon={ShoppingBasket} label="Mini bar / Boutique" />
          ...
          <section className="px-4 pb-24 pt-7 sm:px-8 lg:px-10">
            ...
            <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_360px]">
              <section className="rounded-[25px] bg-white p-5 shadow-sm sm:p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="font-serif text-xl italic">Nouvelle arrivée</p><h3 className="text-lg font-black">INFORMATIONS DU VOYAGEUR</h3></div>
                </div>
                ...
                <div className="mt-6 grid gap-4">
                  ...
                  <label><span className="mb-2 block text-xs font-black">Notes privées</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Lit bébé, arrivée tardive, demande particulière…" rows={4} className="w-full resize-none rounded-[17px] border border-black/10 bg-[#faf8f4] px-4 py-3 text-sm outline-none focus:border-black" /></label>
                </div>

                <button type="button" onClick={prepareStay} className="mt-6 flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-black px-5 text-sm font-black text-white"><KeyRound size={18} /> Générer le lien personnalisé</button>
              </section>
              ...
              <aside className="rounded-[25px] bg-black p-5 text-white sm:p-6">
                ...
                {generatedLink ? (
                  <>
                    <div className="mt-6 rounded-[18px] bg-white/10 p-4"><p className="text-[10px] font-black uppercase tracking-[0.15em] text-white/40">Voyageur</p><p className="mt-1 text-xl font-black">{guestName}</p><p className="mt-3 text-xs text-white/55">{arrivalDate} → {departureDate}</p></div>
                    <p className="mt-5 break-all rounded-[15px] bg-white/10 p-3 text-[11px] leading-relaxed text-white/65">{generatedLink}</p>
                    <button type="button" onClick={shareOrCopyLink} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#eee3d3] text-sm font-black text-black">{copied ? <><Check size={17} /> Message copié</> : <><Share2 size={17} /> Partager ou copier</>}</button>
            ...
            <section className="mt-8">
              ...
              <p className="mt-2 max-w-xl text-sm text-black/45">Les actions principales sont accessibles directement ici.</p>
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                <QuickLink href="/admin/livret" icon={BookOpen} title="Contenu" subtitle="Toutes les rubriques" dark />
                <QuickLink href="/admin/minibar" icon={ShoppingBasket} title="Mini bar" subtitle="Articles, photos et tarifs" />
                <QuickLink href="/admin/livret-a4" icon={Printer} title="Livret A4 / PDF" subtitle="Générer et télécharger" dark />
...
function Field({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string }) {
  return <label><span className="mb-2 block text-xs font-black">{label}</span><input type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="min-h-12 w-full rounded-[17px] border border-black/10 bg-[#faf8f4] px-4 text-sm outline-none focus:border-black" /></label>;
}
...
function QuickLink({ href, icon: Icon, title, subtitle, dark = false }: { href: string; icon: React.ElementType; title: string; subtitle: string; dark?: boolean }) {
  return <Link href={href} className={`flex min-h-[125px] flex-col rounded-[21px] p-4 ${dark ? "bg-black text-white" : "bg-[#eee3d3] text-black"}`}><span className={`grid h-9 w-9 place-items-center rounded-full ${dark ? "bg-white text-black" : "bg-white"}`}><Icon size={18} /></span><div className="mt-auto"><strong className="block text-sm">{title}</strong><span className={`mt-1 block text-[10px] ${dark ? "text-white/50" : "text-black/45"}`}>{subtitle}</span></div></Link>;
}
