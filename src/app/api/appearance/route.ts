...
const RESTORED_CONFIGURATION: GuideHomeConfiguration = { ...DEFAULT_HOME_CONFIGURATION, heroImage: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1400&q=88" };
...
function mergeWithDefaults(configuration: GuideHomeConfiguration): GuideHomeConfiguration {
  ...
  return { ...DEFAULT_HOME_CONFIGURATION, ...configuration, tiles: [...mergedTiles, ...unknownTiles].sort((a, b) => a.position - b.position).map((tile, index) => ({ ...tile, position: index })) };
}
...
function extraConfiguration(raw: Record<string, unknown> | undefined) {
  ...
  return {
    ...
    ...Object.fromEntries(Object.entries(rest).filter(([key]) => !["heroImage", "appIcon", "greeting", "heroTitle", "heroSubtitle", "heroHeight", "whatsappPhone", "tiles"].includes(key))),
  };
}
