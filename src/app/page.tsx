...
import { DEFAULT_GUIDE_CONTENT, mergeGuideContent, type GuideContentConfiguration, type GuideContentSection } from "@/lib/guide-content-config";

type SectionId = keyof typeof DEFAULT_GUIDE_CONTENT | "contact";
type NavigationItem = { label: string; icon: React.ElementType; target: SectionId | null; };
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (appearanceResponse.ok && appearance.success) setHomeConfiguration(appearance.configuration);
      if (contentResponse.ok && content.success) setGuideContent(mergeGuideContent(content.content));
    }
    ...
    <main className="bg-[#e8dfd2]">
      <div className="relative mx-auto w-full max-w-[860px] overflow-hidden bg-[#fbf9f5] shadow-2xl">
        <section className="relative h-[46vh] min-h-[390px] max-h-[590px] overflow-hidden">
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between p-5 text-white"><div className="text-xs font-bold tracking-[0.25em]">THE BEDROOM</div></div>
        ...
        <section className="relative z-10 -mt-5 rounded-t-[30px] bg-[#fbf9f5] px-4 pb-5 pt-7 sm:px-8">
          <div className="mb-6"><p className="font-serif text-2xl italic">Votre livret</p><h2 className="text-2xl font-black tracking-[-0.04em]">LES INFORMATIONS</h2></div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {displayedTiles.map((tile, index) => {
              const Icon = iconById[tile.id] ?? Info;
              return <button key={tile.id} type="button" onClick={() => openTile(tile.id)} className={`relative min-h-[150px] overflow-hidden rounded-[25px] text-left transition active:scale-[0.98] ${tile.size === "full" ? "col-span-2 sm:col-span-3" : tile.size === "large" ? "col-span-2" : "col-span-1"} ${tile.image ? "text-white" : index === 0 ? "bg-black text-white" : "bg-[#eee3d3] text-black"}`}>
                ...
                <div className="relative flex h-full min-h-[150px] flex-col p-4"><span className={`grid h-10 w-10 place-items-center rounded-full ${tile.image ? "bg-white/20 backdrop-blur" : "bg-white text-black"}`}><Icon size={20} /></span><div className="mt-auto"><strong className="block text-[15px] leading-tight">{tile.title}</strong><span className={`mt-1 block text-[11px] ${tile.image || index === 0 ? "text-white/70" : "text-black/50"}`}>{tile.subtitle}</span></div></div>
              </button>;
            })}
          </div>
        ...
        {activeSection && <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-sm"><section className="absolute inset-x-0 bottom-0 mx-auto max-h-[92vh] w-full max-w-[860px] overflow-y-auto rounded-t-[34px] bg-[#fbf9f5] px-5 pb-12 pt-5 sm:px-9">
          <div className="sticky top-0 z-10 mb-5 flex items-center justify-between bg-[#fbf9f5]/95 pb-3 backdrop-blur"><button type="button" onClick={() => setActiveSection(null)} className="flex items-center gap-2 text-sm font-bold"><ArrowLeft size={19} />Retour</button><button type="button" onClick={() => setActiveSection(null)} className="grid h-10 w-10 place-items-center rounded-full bg-[#eee3d3]"><X size={19} /></button></div>
          {section && <SectionContent section={section} copied={copied} onCopy={copyValue} />}
          {activeSection === "contact" && <><Header script="Une question ?" title="CONTACT" icon={MessageCircle} /><p className="mb-5 text-sm leading-relaxed text-black/60">Nous restons disponibles avant et pendant votre séjour.</p><div className="grid gap-3"><a href={`https://wa.me/${homeConfiguration.whatsappPhone}`} target="_blank" className="flex min-h-14 items-center justify-center gap-2 rounded-full bg-black font-bold text-white"><MessageCircle size={19} />Envoyer un message WhatsApp</a><a href={`tel:+${homeConfiguration.whatsappPhone}`} className="flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#eee3d3] font-bold"><Phone size={19} />Appeler</a></div></>}
        </section></div>}
      </div>
    </main>
  );
