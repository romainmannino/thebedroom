        ...
        <section className="relative z-10 -mt-5 rounded-t-[30px] bg-[#fbf9f5] px-4 pb-5 pt-7 sm:px-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {displayedTiles.map((tile, index) => {
              ...
              return (
                <button key={tile.id} type="button" onClick={() => setActiveSection(tile.id as SectionId)} className={`relative min-h-[150px] overflow-hidden rounded-[25px] text-left transition active:scale-[0.98] ${tile.size === "full" ? "col-span-2 sm:col-span-3" : tile.size === "large" ? "col-span-2" : "col-span-1"} ${tile.image ? "text-white" : index === 0 ? "bg-black text-white" : "bg-[#eee3d3] text-black"}`}>
        ...
        {activeSection && (
          <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-sm">
            <section className="absolute inset-x-0 bottom-0 mx-auto max-h-[92vh] w-full max-w-[860px] overflow-y-auto rounded-t-[34px] bg-[#fbf9f5] px-5 pb-12 pt-5 sm:px-9">
              ...
              {activeSection === "contact" && <><Header script="Une question ?" title="CONTACT" icon={MessageCircle} /><p className="mb-5 text-sm leading-relaxed text-black/60">Nous restons disponibles avant et pendant votre séjour.</p><div className="grid gap-3"><a href="sms:+33627630932" className="flex min-h-14 items-center justify-center gap-2 rounded-full bg-black font-bold text-white"><MessageCircle size={19} />Envoyer un message WhatsApp</a><a href="tel:+33627630932" className="flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#eee3d3] font-bold"><Phone size={19} />Appeler le 06 27 63 09 32</a></div></>}
            </section>
          </div>
        )}
      </div>
...
function SectionContent({ section, copied, onCopy }: { section: GuideContentSection; copied: string | null; onCopy: (id: string, value: string) => void }) {
  ...
  return <><Header script={section.script} title={section.title} icon={Icon} /><div className="space-y-4">{section.blocks.map((block) => (
    <article key={block.id} className={`overflow-hidden rounded-[25px] ${block.copyValue ? "bg-black text-white" : "bg-[#eee3d3]"}`}>
